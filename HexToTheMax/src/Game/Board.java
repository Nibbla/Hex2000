...
public class Board {
    ...
    private Game.HexGraph graph;
    private String name = "";
    private LinkedList<String> state;
    
    //For Graphics the board representation
    private Game.Coordinate[][]surfaceNormals;
    public HashMap<Game.IntCoordinate, Game.Coordinate> surfaceNormalsInGood;
    private Game.IntCoordinate lookup = new Game.IntCoordinate(0,0,0);
    private Game.Coordinate zeroInt = new Game.Coordinate(0,0,0);
     ...
     */
    public static Game.Board loadCourse2_5d(String path){
      ...
      //  String[] lines = content.split(System.lineSeparator());
        ...
        c = new Game.Board(name,size,length,width,height, Type.Empty);
            String currentLine;
        ...
        c.calculateSurfaceNormalsSafe();
            ...
            try {
                ...
                bi = Game.DrawPanel.createImage(c);
                Game.Utils.saveManagedBufferedImage(Config.CourseLocation + c.name + ".png",bi);
            }finally {
    ...
    public void saveCourse2_5D() {
        ...
        if (managedBufferedImage==null){//(true){//
            managedBufferedImage = Game.DrawPanel.createImage(this);
        }
        Game.Utils.saveManagedBufferedImage(Config.CourseLocation + name + ".png", managedBufferedImage);
    ...
    private void addTriangularPrism(Game.IntCoordinate c1, Game.IntCoordinate c2, Game.IntCoordinate c3, int height, Type t){
        //todo
    }
    ...
    public Game.Coordinate getNormal(int x, int y, int z) {
        ...
        if (avgX==0&&avgY==0&&avgZ==0)return new Game.Coordinate(0, 0, 0);
       /* double length =  Game.Utils.fastInverseSqrt(avgX * avgX + avgY * avgY + avgZ * avgZ);//Math.sqrt(); // distance from avg to the center
        ...
        avgZ /= length;
        avgY /= length;
        return new Game.Coordinate(avgX, avgY, avgZ);
    }
    public void calculateSurfaceNormals(){
        calculateSurfaceNormalsHashMap();
        surfaceNormals = new Game.Coordinate[dimension[0]][dimension[1]];
    ...
    private void calculateSurfaceNormalsHashMap() {
        surfaceNormalsInGood = new HashMap<Game.IntCoordinate, Game.Coordinate>((int) (dimension[0]*dimension[1]*4.4));
        for (int x = 0; x <dimension[0];x++){
            yLoop:for (int y = 0; y <dimension[1];y++){
                int z= dimension[2]-1;
                while (true){
                    ...
                    if (playfield[x][y][z] != Type.Empty){
                        Game.Coordinate c = getNormal(x, y, z);
                        if (c.getLength()==0) {
                            continue yLoop;
                        }
                        surfaceNormalsInGood.put(new Game.IntCoordinate(x, y, z), getNormal(x, y, z));
                    }
                    z--;
                }
            }
    ...
    public void calculateShadingMap(){
        ...
        for (int x = 0; x <dimension[0];x++){
            for (int y = 0; y <dimension[1];y++){
                    //angle = acos(v1•v2)
                     Game.Coordinate c = surfaceNormals[x][y];
    ...
    public Game.Coordinate[][] getSurfaceNormals() {
    ...
    public void finalise() {
        ...
        System.gc();
        setBufferedImage(Game.DrawPanel.createImage(this));
        System.gc();
    }

    public void integrate(Game.Board previewMiniBoard, int x, int y) {
    ...
    public Type getType(Game.Coordinate newPosition) {

        return null;
    }
    public Game.Coordinate getNormalQuick(int x, int y, int z) {
        if (surfaceNormalsInGood==null) calculateSurfaceNormalsHashMap();
        lookup.set(x, y, z);
        Game.Coordinate c = surfaceNormalsInGood.get(lookup);
    ...
    public Game.HexGraph getGameSate() {
        return graph;
    }
}